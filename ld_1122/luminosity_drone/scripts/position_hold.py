...
class swift():
	...
	def __init__(self):
		...
		self.setpoint = [1.7,2,20] # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly
		...
		self.Kp = [100.099999, 0, 11.399999999999999]
		self.Ki = [0.945, 0, 0.060]
		self.Kd = [516, 0, 380]
	...
	def pid(self):
		...
		# if(self.alt_error[2] > 0.2 or self.alt_error[2]<-0.2):
		# 	rospy.loginfo("<------####------>")
		...
		if(self.alt_error[0] > .1 or self.alt_error[0]<-.1):
			rospy.loginfo("<------####------>")
