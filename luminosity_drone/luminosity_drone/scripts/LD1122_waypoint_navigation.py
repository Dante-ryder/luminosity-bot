...
# Functions:        < disarm(self), arm(self), whycon_callback(self,msg), altitude_set_pid(self,alt), pitch_set_pid(self,pitch), roll_set_pid(self,roll), def pid(self, setpoint)>
# Global variables: < setpoints, drone_position, pos, cmd, kp, ki, kd, min_value, max_value, alt_err, prev_error, sum_alt_error, publisher(alt, prev_alt, sum_alt errors) >
'''
...
class swift():
	...
	def __init__(self):
		...
		rospy.init_node('waypoint_navigation', anonymous = True)	# initializing ros node with name waypoint_navigation
		...
		# self.pos to iterate over the setpoints
		self.pos = 0
	...
	def pid(self, setpoint):
		if self.alt_error[0] < 0.2 and self.alt_error[0] > -0.2 and self.alt_error[1] < 0.2 and self.alt_error[1] > -0.2 and self.alt_error[2] < 0.2 and self.alt_error[2] > -0.2:
			...
			if(self.helper==20 and self.pos<=9):
				...
				rospy.loginfo("Pos : " + str(self.pos))
				rospy.loginfo("Drone is at : " + str(self.drone_position))
